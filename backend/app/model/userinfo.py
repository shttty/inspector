...
def premission(username, domain):
    ...
    cursor.close()
    connection.close()
    if row1.get("isadmin") == 1:
        return True
    else:
        return False


def onDuty(username, domain):
    connection = linkStart()
    cursor = connection.cursor(dictionary=True)
    sql = f"SELECT isonduty FROM inspectoruser WHERE username='{username}' and domain='{domain}'"
    cursor.execute(sql)
    results = cursor.fetchall()
    if len(results) == 0:
        return False
...
if __name__ == "__main__":
    print(getUserOwn("sunheng", "strategy"))
